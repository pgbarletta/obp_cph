...
# Generated from .ipynb template: eol_rmsf.ipynb
...
dir_eol_rmsf = string(home, "run/eol/rmsf/")
...
cell_2=r"""
for i in 1:10
    global rmsf_eol_idx = readdlm(joinpath(dir_eol_rmsf, string(i),
            string("rmsf_eol_", idx[i])))[2:end, 2]
    global rmsf_eol_ph = readdlm(joinpath(dir_eol_rmsf, string(phs[i], "ph"),
            string("rmsf_eol_", phs[i])))[2:end, 2]

    sym_rmsf_eol_idx = Symbol("rmsf_eol_", idx[i])
    sym_rmsf_eol_ph = Symbol("rmsf_eol_", phs[i])
    eval(:($sym_rmsf_eol_idx = rmsf_eol_idx))
    eval(:($sym_rmsf_eol_ph = rmsf_eol_ph))
end
""".strip()
...
for ph in range(30, 80, 5):
    celda_ph = r"""plot(collect(1:aa), rmsf_eol_""" + str(ph) + r""",
...
for idx in range(1, 11):
    celda_idx=r"""plot(collect(1:aa), rmsf_eol_""" + str(idx) + r""", 
...
nbf.write(nb, r'/home/pbarletta/labo/20/cph_obp/src/informe_eol_rmsf.ipynb')
print(r"Jupyter notebook /home/pbarletta/labo/20/cph_obp/src/informe_eol_rmsf.ipynb successfully generated.")
