...
# Generated from .ipynb template: apo_vol.ipynb
...
for i in 1:10
    ...
    global vol_apo_phs = convert(Array{Float64, 1}, readdlm(joinpath(dir_apo_vol, string(phs[i], "ph"),
            string("vol_trj_apo_", phs[i])))[:, 3])

    sym_vol_trj_apo_idx = Symbol("vol_trj_apo_", idx[i])
    sym_vol_trj_apo_phs = Symbol("vol_trj_apo_", phs[i])
...
for ph in range(30, 80, 5):
    celda_ph=(r"""plot(collect(1:nstlim) ./ 100, vol_trj_apo_""" + str(ph) + r""", 
    ...
    celda_ph_his=(r"""
        ...
        wgh_vol = fill(1.0, length(vol_trj_apo_""" + str(ph) + r"""))
        
        # Histograma ponderado
        Wbins_vol, Whis_vol = JUMD.weightedHist(vol_trj_apo_""" + str(ph) + r""", bin_vol, wgh_vol, true, false);
...
for ph in range(30, 80, 5):
    if ph == 75:
        ...
        [ mean(vol_trj_apo_""" + str(ph) + r""") std(vol_trj_apo_""" + str(ph) + r""") std(vol_trj_apo_""" + str(ph) + r""") / mean(vol_trj_apo_""" + str(ph) + r""") ])""").strip() + "\n" + "\n"
        break
...
for idx in range(1, 11):
    celda_idx=(r"""
    plot(collect(1:nstlim) ./ 100, vol_trj_apo_""" + str(idx) + r""", 
    ...
    st = 20
    bin_vol = collect(0:st:max_vol)
    wgh_vol = fill(1.0, length(vol_trj_apo_""" + str(idx) + r"""))
    
    # Histograma ponderado
    Wbins_vol, Whis_vol = JUMD.weightedHist(vol_trj_apo_""" + str(idx) + r""", bin_vol, wgh_vol, true, false);
    ...
    celda_tabla_idx=(r"""NamedArray([ mean(vol_trj_apo_""" + str(idx) + r""") std(vol_trj_apo_""" + str(idx) + r""") std(vol_trj_apo_""" + str(idx) + r""") / mean(vol_trj_apo_""" + str(idx) + r""") ],
...
for idx in range(1, 11):
    if idx == 10:
        ...
        [ mean(vol_trj_apo_""" + str(idx) + r""") std(vol_trj_apo_""" + str(idx) + r""") std(vol_trj_apo_""" + str(idx) + r""") / mean(vol_trj_apo_""" + str(idx) + r""") ])""").strip() + "\n" + "\n"
        break
...
nbf.write(nb, r'/home/pbarletta/labo/20/cph_obp/src/informe_apo_vol.ipynb')
print(r"Jupyter notebook /home/pbarletta/labo/20/cph_obp/src/informe_apo_vol.ipynb successfully generated.")
