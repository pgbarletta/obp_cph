...
# Generated from .ipynb template: ctv_hbo.ipynb
...
for ph in range(30, 80, 5):
    ...
    max_nhb = 90
    plot(collect(1:nstlim) ./ 100, nhb_ctv_""" + str(ph) + r""",
    ...
    wgh_nhb = fill(1.0, length(nhb_ctv_""" + str(ph) + r"""))
    
    # Histograma ponderado
    Wbins_nhb, Whis_nhb = JUMD.weightedHist(nhb_ctv_""" + str(ph) + r""", bin_nhb, wgh_nhb, true, false);
...
for ph in range(30, 80, 5):
    if ph == 75:
        ...
        [ mean(nhb_ctv_""" + str(ph) + r""") std(nhb_ctv_""" + str(ph) + r""") std(nhb_ctv_""" + str(ph) + r""") / mean(nhb_ctv_""" + str(ph) + r""") ])""").strip() + "\n" + "\n"
        break
...
for idx in range(1, 11):
    celda_idx=(r"""
    plot(collect(1:nstlim) ./ 100, nhb_ctv_""" + str(idx) + r""",
    ...
    st = 2
    bin_nhb = collect(min_nhb:st:max_nhb)
    wgh_nhb = fill(1.0, length(nhb_ctv_""" + str(idx) + r"""))
    
    # Histograma ponderado
    Wbins_nhb, Whis_nhb = JUMD.weightedHist(nhb_ctv_""" + str(idx) + r""", bin_nhb, wgh_nhb, true, false);
...
for idx in range(1, 11):
    if idx == 10:
        ...
        [ mean(nhb_ctv_""" + str(idx) + r""") std(nhb_ctv_""" + str(idx) + r""") std(nhb_ctv_""" + str(idx) + r""") / mean(nhb_ctv_""" + str(idx) + r""") ])""").strip() + "\n" + "\n"
        break
...
nbf.write(nb, r'/home/pbarletta/labo/20/cph_obp/src/informe_ctv_hbo.ipynb')
print(r"Jupyter notebook /home/pbarletta/labo/20/cph_obp/src/informe_ctv_hbo.ipynb successfully generated.")
